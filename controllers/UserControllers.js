...
async function createRecord(req, res) {
  if (schema.validate(req.body.password)) {
      ...
      if (error) {
        ...
        try {
          ...
          error.keyValue?.username
            ? (errorMessage.username = "User With This User Name Already Exist")
            : null;
    ...
    res.status(400).send({
      result: "Fail",
      reason:
        "Invalid Password. It Must Container at least 1 upper case and 1 lower case alphabet, 1 digit, should not contain any space and length must be 8-100",
    });
  }
}
...
async function updateRecord(req, res) {
  try {
    ...
    error.keyValue?.username
      ...
      : null;
    error.keyValue?.email
      ? (errorMessage.email = "User With This Email Already Exist")
      : null;
  ...
  try {
    ...
    if (data) {
      if (await bcrypt.compare(req.body.password, data.password)) {
        ...
        jwt.sign({data}, key, { expiresIn: "10 days" }, (error, token) => {
          if (error) {
            res.status(500).send({
              result: "Fail",
              reason: "Internal Server Error",
